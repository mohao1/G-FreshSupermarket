...
func (l *ReleaseProductLogic) ReleaseProduct(in *bms.ReleaseProductReq) (*bms.ReleaseProductResp, error) {
	...
	err = l.svcCtx.TransactCtx(l.ctx, func(ctx context.Context, session sqlx.Session) error {
		...
		//插入商品数据
		_, err = l.svcCtx.ProductListModel.TransactInsert(ctx, session, &data)
