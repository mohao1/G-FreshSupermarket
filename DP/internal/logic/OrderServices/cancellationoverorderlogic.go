...
func (l *CancellationOverOrderLogic) CancellationOverOrder(req *types.CancellationOverOrderReq) (resp *types.DataResp, err error) {
	//获取JWT中的UserId
	userId := fmt.Sprint(l.ctx.Value("jwtUserId"))

	//调用RPC的服务
	order, err := l.svcCtx.SmsRpcClient.CancellationOverOrder(l.ctx, &smsclient.ConfirmOrderReq{
		UserId:      userId,
